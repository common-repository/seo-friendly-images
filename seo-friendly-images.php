...
Plugin URI: https://www.dropbox.com/s/alihg9cekqpq6ek/seo-friendly-images.zip
Description: SEO Friendly Images automatically adds alt and title attributes to all your images improving traffic from search engines.
Version: 1.0.0
Author: scottie
Author URI:
...
*/
	add_filter('the_content', 'seo_opt_image_replace_tags', 100);	
//	add_filter();
   
	    
function seo_opt_image_replace_tags ($content, $alt_text='',$title='')
{   
    
    
    global $post; 
   
   $seo_opt_image_options_array = get_option('seo_opt_image_options_values'); 
   
   $alt_text = $seo_opt_image_options_array['seo_opt_image_alt_value'];
   $title_text = $seo_opt_image_options_array['seo_opt_image_title_value'];
   ...
   $alt_flag = $seo_opt_image_options_array['seo_opt_image_override_alt_value']; 
   
      //check setting for overinding title tag
   $title_flag = $seo_opt_image_options_array['seo_opt_image_override_title_value']; 
...
{
add_menu_page( 'seo_opt_image_settings_page', 'Seo Friendly Images', 'administrator', 'seo_opt_image_setting','seo_opt_image_create_setting_page',''); 
 }
 
 function seo_opt_image_create_setting_page()
{
 require_once('seo-friendly-images-settings.php');
}


add_action( 'admin_enqueue_scripts', 'seo_opt_image_load_custom_wp_admin_style' ); 
 
 function seo_opt_image_load_custom_wp_admin_style($hook) {
   if ($hook != 'toplevel_page_seo_opt_image_setting'){return;} // we dont want to load our css on other pages
        wp_register_style ('seo_opt_image_custom_wp_admin_css', plugins_url('css/plugin-admin-panel.css', __FILE__));
        wp_enqueue_style( 'seo_opt_image_custom_wp_admin_css' );
 ...
 function seo_opt_image_load_custom_wp_admin_scripts($hook) {
  
  
  if ($hook != 'toplevel_page_seo_opt_image_setting'){return;} // we dont want to load our js on other pages
  
  wp_register_script( 'seo_opt_image_custom_wp_admin_js', plugin_dir_url( __FILE__ ) . 'js/plugin-admin-panel.js',array('jquery','jquery-ui-core','jquery-ui-tabs','wp-color-picker'), false, '1.0.0' );
 wp_enqueue_script ('seo_opt_image_custom_wp_admin_js');

       
}
add_action( 'admin_enqueue_scripts', 'seo_opt_image_load_custom_wp_admin_scripts' );
